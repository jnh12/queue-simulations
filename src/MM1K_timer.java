...
public class MM1K_timer {
    ...
    static final int QMAX = 1;
    static final double theta = 0.90; // rate for deadlines
    ...
    static void handleArrival(State s) {
        ...
        if (!s.repairman) {
            ...
            s.curArr = s.MC;
            s.curSrvStart = s.MC;
            s.curDeadline = s.MC + expRate(theta);  // deadline for the job that just started service
    ...
    static void handleDeparture(State s) {
        if (!Double.isNaN(s.curDeadline) && s.curDeadline <= s.MC) {
            s.expiredAny++;
        }
        s.departed++;
